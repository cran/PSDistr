...
#' @return The function returns the value of the cumulative distribution function for the plasticizing component.
#' @rdname ppc
#'
#' @details
...
ppc <- function(x, teta, s2, c) {
  pnorm <- NULL; sign <- NULL; abs <- NULL
  if (s2 > 0 & c >= 1)
  {
    e <- (x - teta) / s2
    return(pnorm(sign(e) * abs(e) ^ c, 0, 1))
  }
  else
  {
    return('s2>0 and c>=1')
