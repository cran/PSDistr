...
#' @return The function returns the value of the cumulative distribution function for the Expnormal distribution.
#' @rdname pen
#'
#' @details
...
pen <- function (x, a1, b1, a2, b2, c) {
  pnorm <- NULL
  if (b1 > 0 & b2 > 0)
  {
    return(pnorm(c - exp((a1 - x) / b1) + exp((x - a2) / b2), 0, 1))
  }
  else
  {
